...
//    version = [version substringToIndex:5];
	return version;
#else
	return nil;
#endif
...
+(BOOL)isFromRomotoNoticication{
    ...
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"isRemoto"];

}

+(void)saveFlagIsFromRomotoNoticication:(BOOL)romoto {
   
    [[NSUserDefaults standardUserDefaults] setBool:romoto forKey:@"isRemoto"];
    [[NSUserDefaults standardUserDefaults] synchronize];
...
+ (BOOL)isPhoneRetina6{
#if (TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR)
    return [XTSystem isScreenSize:CGSizeMake(750, 1134)];
#else
    return NO;
#endif
}
